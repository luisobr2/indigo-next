...
export const QUERY_MODELS: Record<string, ModelSpec> = {
  ...
  "indigo.payout": {
    label: "Liquidaciones a contratistas (el instalador cobra por jornada: work_date es el dia)",
    fields: PAYOUT_FIELDS,
    defaultFields: ["id", "name", "contractor_id", "contractor_type", "state", "amount", "work_date"],
    defaultOrder: "create_date desc",
  },
  "indigo.payout.line": {
    label: "Renglones de cada liquidación. line_kind: 'work' sale de una orden instalada; 'minimum' y 'bonus' los calcula la regla del día — al contar puertas, filtrá line_kind='work'",
    fields: PAYOUT_LINE_FIELDS,
    defaultFields: ["id", "payout_id", "order_id", "line_kind", "description", "quantity", "rate", "amount"],
    defaultOrder: "id desc",
  },
  "indigo.stage": {
