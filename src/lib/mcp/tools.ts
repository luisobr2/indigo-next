...
// ---------------------------------------------------------------------
// Limit clamp — the blast-radius cap. Never trust the caller's number.
// ---------------------------------------------------------------------
...
const LIMIT_SCHEMA_PROPERTY = {
  type: "number",
  description:
    "Maximum number of records to return. Defaults to 25. Capped at 100 server-side no matter what is requested — ask a narrower question (add a filter) instead of raising this.",
};
...
export const TOOL_DEFS: ToolDef[] = [
  {
    ...
    description:
      "Shows every order currently in an active production stage (excludes Invoiced/Paid and Closed), grouped by stage in pipeline order. Use this to answer 'what's on the board right now' questions like '¿qué hay para pintar hoy?' or 'what's in CNC today' without paging through the full order list. Returns one group per stage that has work in it, each with the matching orders (client, dealer, door count, install date).",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    title: "Find orders",
    description:
      "Searches orders by free text (matches order number, client name or dealer reference, case-insensitive partial match) and/or an exact stage code or dealer id. Use this when the user names a customer, an order number like 'IO-0007', a dealer reference, or wants orders in a specific stage — e.g. 'find Perez's order' or 'Lock Tight orders in painting'. Returns the most recently created matches first. Use list_stages or list_dealers first if you need the exact code/id.",
    inputSchema: {
      type: "object",
      properties: {
        q: {
  ...
  },
  {
    ...
    description:
      "Lists the production pipeline stages (New Order through Closed) in their configured order, each with its numeric id, display name and short code (e.g. 'painting', 'cnc'). Stage codes are configurable per deployment, so call this before filtering by stage anywhere else instead of guessing a code — it's the ground truth for what stages actually exist right now.",
    inputSchema: {
      type: "object",
      properties: {
        limit: LIMIT_SCHEMA_PROPERTY,
