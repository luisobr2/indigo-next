...
export default function DealerDetailPage({
  ...
  async function save() {
    setBusy(true);
    // Toast strategy: show a "Saving..." promise toast that auto-replaces
    // itself with success/error based on the await. One UI moment, no
    // double notifications, accessible (sonner announces via aria-live).
    const promise = fetch(`/api/catalog/dealers/${id}`, {
      method: "PUT",
  ...
  if (!data?.dealer)
    return (
      <ErrorState
        ...
        message={`Dealer #${id} doesn't exist or you don't have permission to see it.`}
        backHref="/catalog"
      />
    );
  ...
  return (
    <div className="mx-auto max-w-[1300px] space-y-5">
      <div className="flex items-center gap-2 text-sm text-slate-500">
        ...
        <span>&rsaquo;</span>
        <span className="font-medium text-slate-800">{data.dealer.name}</span>
      </div>
